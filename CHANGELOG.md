...
- **LLM chronic-condition ICD label hallucination fixed by resolving display at context-build time** (#1446, closes #1445). The LLM narrative pipeline (vLLM + Qwen3.8-27B-FP8) had received `chronic_conditions` payload as bare codes with no paired display, and the model resolved the label from parametric memory — sometimes wrong. On the H100 p=100 verify 3 US narrative docs rendered `G20` (Parkinson's disease) as "glaucoma" (correctly H40), which silently disconnected the patient's antiparkinson medication (Levodopa/Carbidopa) from its indication. Fix: `_render_chronic_list` in `narrative/replacement_strategy.py` now resolves the display via `clinosim.codes.lookup(system, code, lang)` when the entry carries a `system` field, rendering as `"<display> (<code>) (<stage>, <severity>)"`; entries without `system` (legacy CIFs) pass through unchanged, and unresolved codes fall back to code-echo without emitting a redundant `"CODE (CODE)"` string. `narrative_seed_bundle.yaml` (both EN + JA) bumped to v22 with a new Rule 3 VERBATIM COPY sub-clause on ICD-coded conditions: the paired display is authoritative; the LLM MUST NOT resolve labels from parametric memory. Verified on a fresh H100 narrate: G20 → "Parkinson disease" (0 "glaucoma" hits across p=100 seed=400 + seed=401 US cohorts); E11 / M17 / F32 / J44 / N18 / I10 now emit their full ICD-10-CM labels ("Osteoarthritis of knee", "Depressive episode", "Chronic obstructive pulmonary disease", etc.) instead of the pre-fix generic simplifications. Narrative-CIF drift; would qualify as MINOR under the strict policy but bundled into this release per maintainer sign-off.
- **`Encounter.period.end` clamp + `dischargeDisposition` flip when past `Patient.deceasedDateTime`** (#1443, closes #1442). `_drop_entries_after_death` (`clinosim/modules/output/fhir_r4/__init__.py`) gated Encounter on `period.start` only per the pre-existing #1219 reasoning (in-hospital deaths legitimately overshoot dod for `disp=exp`), which fails for `disp=home` inpatient stays whose LOS-driven discharge date happens to land after the actuarial `natural_death` date — the sim then emitted a live "home" discharge on a day the patient was already dead. Observed on p=10000 s=362: US 1 / JP 2 cases with `period.end` 6-18 days past dod. Fix mirrors the sibling Coverage branch (#1278): clamp `period.end` down to dod, flip a non-exp `dischargeDisposition` to `exp`. Encounters already at `disp=exp` keep their code untouched but still get the clamp so strict datetime consumers see a consistent timeline. FHIR emit-only, CIF unchanged (PATCH).
- **`Patient.deceasedDateTime` expands to end-of-day for date-only inputs** (#1441, closes #1440). `clinosim/modules/output/fhir_r4/demographics/patient.py` previously forwarded `date_of_death` via `str(_dod)`, so a `date`-only value emitted as bare `YYYY-MM-DD`. Strict-mode FHIR consumers expand the missing time to `T00:00:00`, and every same-day intra-day event after 00:00 (encounter discharge, post-mortem Condition record) then looks post-mortem under a strict datetime compare. Observed on p=10000 s=361: 1 JP case (`Condition.recordedDate 2026-09-01T00:42+09` vs `deceasedDateTime 2026-09-01`). New helper `to_fhir_deceased_datetime(value, country)` in `fhir_r4/lib/common.py` expands date-only values to `T23:59:59{TZ}` (JP → `+09:00`, other → `Z`) and passes full datetime inputs through unchanged. Internal after-death entry gate is unaffected (already compares by calendar-date prefix). FHIR emit-only, CIF unchanged (PATCH).
- **Nursing / ADL / intake-output Observations emit with encounter-anchored time** (#1439, closes #1437). Barthel / Braden / Morse / intake-total / urine-output / output-total Observation records in `clinosim/modules/output/fhir_r4/procedures/nursing.py` forwarded the CIF `date`-only field through `to_fhir_datetime`, which returned the plain `YYYY-MM-DD` string. FHIR-consuming exporters expand this to `T00:00:00`, so every day-0 assessment landed at midnight of the admission date — before the encounter's actual `period.start` for any admission dispatched later in the day. Observed on p=10000 s=360: US 3,288 / JP 3,894 Observations across the six categories, all violating `Observation.effectiveDateTime >= Encounter.period.start`. New emit-time helper `_resolve_assessment_effective(assessment_date, encounters)` combines the assessment date with the encounter's admission time on day 0 (nurse assessment on arrival) and with a nurse-shift-plausible hour (09:00) on later days, preserving the encounter's timezone. CIF schema unchanged (`ADLAssessment.date`, `NursingRiskAssessment.date`, `IntakeOutputRecord.date` remain `date`). FHIR emit-only (PATCH).
- **Post-hoc overlap verifier wired into perinatal / abortion / unknown_condition dispatchers** (#1438, closes #1436). The verifier landed in #1434 was wired at 2 of 5 `patient_records` append/extend sites in `simulator/engine.py`; three sibling dispatchers (delivery, abortion, unknown_condition) bypassed it. Observed on p=10000 s=360: 1 JP case where a mother hospitalized 08-23 for asthma (J45.9) had her delivery (O80) fire 08-26 and open a physically-impossible concurrent inpatient encounter. New `_handle_overlap_via_merge(patient_records, new_record, disease_id, companions=None)` helper consolidates the verifier + merge pattern and adds a companion-record `admit_source_encounter_id` (FHIR `Encounter.partOf`) rewrite — when a mother's delivery encounter is merged into an ongoing stay, the newborn record's partOf reference is repointed to the surviving encounter so the linkage does not dangle. Wired into all three previously-bypassed sites; the existing #1434 sites are unchanged (mid-loop pattern already handles their bed-occupancy undo). CIF drift (RNG cascade risk on the merge path; MINOR).
- **Newborn workup one-liner rendered in HPI section** (#1435, closes #1432). `template_generator._build_hpi` did not consume `ctx.newborn_workup` — the context wiring reached the LLM prompt grounding via `replacement_strategy._build_extra_context`, but the deterministic template body never emitted Apgar / bilirubin / CCHD / screen-flag content. On p=10000 s=359 the 851 US / 852 JP admission_hp Compositions for Z38.0 encounters carried zero `Apgar` references in FHIR-emitted `section.text.div`. Fix wraps `_build_hpi` so every return path picks up a newborn workup append; the append reuses `replacement_strategy._render_newborn_workup_summary` (the same helper the LLM prompt uses) so template body and LLM narrative anchor on identical CIF-derived facts. Non-neonate records pass through unchanged. Narrative-CIF drift (MINOR).
- **`safety_skips` + `newborn_workup` wired into production `NarrativeContext` factory** (#1433, closes #1431, refs #1432). Two divergent `NarrativeContext` factories had drifted: the unit-test factory (`context.build_narrative_context`) was fully wired, but the production factory (`passes.NarrativePass._build_context`) silently omitted `safety_skips=` and `newborn_workup=` — every entry / projection fell to the dataclass default and dropped between CIF and rendered narrative. Observed on p=10000 s=359: US 4,927 / JP 4,653 `patient.safety_skip_log` entries surfaced zero addendum lines in 16,865 US / 44,271 JP DocumentReference documents; neonatal Apgar/CCHD/screen projection had zero hits on all 851 US / 852 JP admission_hp Compositions. `passes.py:_build_context` now delegates to `context.py:_build_safety_skips` + `context.py:_build_newborn_workup`; `_build_safety_skips` was refactored from `getattr`/attribute access to the shared `_o` (`get_attr_or_key`) helper so the same builder accepts both attribute-shaped inputs (in-process `SafetySkipEntry` dataclass + `Encounter`) and dict-shaped inputs (JSON-hydrated CIF read from disk by `passes.NarrativePass`). Narrative-CIF drift (MINOR).
- **Post-hoc actual-period overlap verifier for inpatient dispatch** (#1434, closes #1430). `simulator/engine.py` pre-dispatch gates (`_find_active_inpatient_record` in the main loop, `_find_overlapping_inpatient_record` in the readmission loop) key off `event_time = datetime(year, month, day, 12, 0)` because the life-event stream carries date-precision timestamps; the actual admit hour (decided inside `_simulate_patient`) is not visible to those gates, so hour-level overlaps escape. Observed on p=10000 s=359: 1 US case (two same-day life events both landing after noon → 18:03 + 19:46 concurrent admits) and 1 JP case (readmission at 06:12 landed before prior stay's 10:37 discharge). New `_verify_no_actual_overlap(patient_records, new_record)` helper runs after `_simulate_patient` returns and before the record is appended, using the actual admission_datetime / discharge_datetime; overlap detected → merge as in-hospital complication via `_merge_disease_into_active_encounter` (existing #848 pattern), reverse the bed-occupancy / concurrent-patients increments, skip append. Wired into both the main dispatch loop and the readmission loop. CIF drift (RNG cascade risk on the merge path; MINOR).

### Changed

- **CI: nightly `.test_durations` regeneration from ubuntu runner** (S115, follow-up to #1428). New `test-durations-regen` job in `nightly.yml` runs `pytest --store-durations` for unit + integration on ubuntu-latest and opens a routine "chore(ci): refresh .test_durations" PR when the file drifts. Motivation: post-#1428 install variance was solved by uv but the residual integration-shard imbalance (max 6m29s vs min 2m22s, gap ~4 min) was 100 % test-time distribution because `.test_durations` had been recorded on a maintainer M-series Mac where sim-heavy tests run ~2-3× faster than on ubuntu-latest — so `pytest-split`'s bin-pack looked perfect on paper (1.01× at N=10) but leaned heavy on the shards that landed the sim-bound tests. Regenerating from the same runner infra as PR CI aligns bin-pack with reality; expected max shard drop from ~6.5 min toward the theoretical floor once the first nightly refresh lands.
- **CI: introduce `uv` for dependency installation across all workflows** (S115, follow-up to Issue #1420). Replace `pip install` with `uv pip install --system` in all 9 workflows (ci / nightly / release / docs / jp-clins-lab-compliance-gate / jp-core-compliance-gate / us-core-compliance-gate / jp-validate / us-validate). Each workflow now inserts an `astral-sh/setup-uv@v6` step immediately after `actions/setup-python`, with `enable-cache: true` and `cache-dependency-glob: pyproject.toml` so the wheel cache keys off the dependency spec. The unit-test shard imbalance observed post-#1426 (max 4m5s vs min 1m36s, gap ~2m30s) is dominated by pip-install runner-cache variance (~3m cold vs ~30s warm); `uv` installs the same wheels ~10× faster, so the install phase converges to ~30s regardless of cache state. Expected gap collapse: shard variance drops from 2-3 min to ~20-30s, so the max-shard wall matches the min-shard wall much more tightly. `cache: pip` on `setup-python` and any `python -m pip install --upgrade pip` steps are removed (uv owns its own cache and does not need a bootstrapped pip).
...
- **IV cycle-based chemotherapy agents (Oxaliplatin / Pemetrexed /
  ...
  ``chronic_medications.yaml`` C18 / C34 / C50 blocks listed IV chemo
  drugs as chronic monthly meds with independent-Bernoulli sampling —
  a colon-cancer patient could sample Oxaliplatin 130 mg as a
  standalone chronic MR every ~34 days without its FOLFOX partners
  (5-FU / Leucovorin), producing clinically-nonsensical monotherapy
  emit. At p=10k s356 US, Oxaliplatin fired 652 times on
  non-chemo-visit encounters vs 456 5-FU / 456 Leucovorin — the
  surplus 196 standalone MRs matched the buggy chronic-med path.

  Fix: scoped the oncology block in ``chronic_medications.yaml`` to
  ORAL / SUBCUTANEOUS daily home chemo only (Capecitabine, Osimertinib,
  Sorafenib, Lenvatinib, Tamoxifen, Anastrozole, Bicalutamide,
  Leuprorelin depot). IV cycle drugs (Oxaliplatin, Pemetrexed,
  Carboplatin, Trastuzumab) emit exclusively via ``chemo_regimens.yaml``
  cycle scheduling (FOLFOX q14d, CarboPem q21d). Also partially
  addresses Issue #1329 (Capecitabine + IV 5-FU concurrent) by removing
  the second IV 5-FU path from the chronic-med emit.

  Verification (p=500 s356 US): all 12 Oxaliplatin MRs now sit on
  encounters with reasonCode "Malignant neoplasm of colon, unspecified"
  (FOLFOX regimen path). Pemetrexed / Carboplatin / Trastuzumab MRs on
  the chronic-med path: 0 (was 0.3 × colon-cancer, 0.3 + 0.2 ×
  lung-cancer, 0.15 × breast-cancer distributions).

  Non-scope: Trastuzumab as a full regimen entry in
  ``chemo_regimens.yaml`` (HER2-directed weekly / q3wk cycle) is a
...
- **Inpatient progress_note subjective repeats identical boilerplate
  ...
  ``_compose_progress_subjective_from_state`` fallback emitted the
  same "自覚症状に著変なし。" / "No new subjective complaints."
  string on every day of a multi-day stay. Exemplar pt-25a61e67a2e2
  (JP, 99yo F, 15-day I50.9 CHF admission) produced 15 identical daily
  notes despite the CIF containing a Cr 4.26 → 0.84 recovery and
  dyspnea improvement trend.

  Fix: (a) added a yesterday-vs-today trend comparison — a resolved
  fever now renders "熱型下降 (前日 38.6°C → 本日 37.0°C)。" instead
  of falsely repeating "Persistent fever"; a new-onset fever renders
  "発熱 38.6°C 出現。"; a recovering SpO2 renders "SpO2 改善 (前日
  88% → 本日 95%)。"; (b) when today has no abnormal signal, pick a
  phase-aware neutral phrase from a small pool (early / mid / late /
  discharge-eve) rotated deterministically by ``day_index`` so
  consecutive days differ. Every phrase describes the clinical hold
  ("食事摂取良好、明らかな苦痛の訴えなし。" / "Continuing planned
  therapy; symptoms broadly stable.") without asserting an unmodeled
  symptom — a nurse's neutral-observation vocabulary, not fabrication.
  Rotation is deterministic (no RNG consumed) so seed reproducibility
  is preserved.

  Verification: 10-day stable stay now shows ≥3 distinct subjective
  bodies (was 1). Trend phrases surface on the correct day.
