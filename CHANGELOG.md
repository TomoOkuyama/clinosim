...
- **CI: replace `pip install` with `uv pip install --system` in all 9 workflows** (S115). Adds `astral-sh/setup-uv@v6` (cache keyed to `pyproject.toml`) after every `actions/setup-python` step. Motivation: post-#1426 unit-shard gap 2m30s was dominated by pip-install runner-cache variance (~3m cold vs ~30s warm), not test distribution. `uv` installs ~10× faster and converges the install phase to ~20-30s regardless of cache state.
