...
- **Phase 3a 3-helper flag merge** — `derive_lab_values` reads THREE flag dicts: `{**scenario_flags_from_protocol(p), **medication_flags_from_context(...), **hai_flags_from_record(record, encounter_id, day)}`. Call sites merge all three via `**flags`. NEVER add a flag=value directly at a call site — extend the appropriate helper. The `hai_flags_from_record` helper is currently a primitive for Phase 3b reuse; Phase 3a itself uses the sibling `apply_hai_lab_lift` (in `clinosim/modules/hai/lab_lift.py`) which walks `record.extensions["hai"]` after the daily loop completes and adds a forward-formula delta to existing WBC + CRP `obs.value` using per-day `state_history` snapshots (so the original noise + circadian is preserved). Phase 3b/c will reuse the same forward-delta pattern for antibiotic decay and Lactate / Plt / Temp / SBP sepsis cascade extensions.
