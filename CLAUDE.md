...
- **`lru_cache` maxsize convention (PR-A 2026-06-26, PR-B1 2026-06-27 拡張)** — `load_X()` no-param → `maxsize=1`; `load_X(country: str)` → `maxsize=2` (US + JP); `load_X(country, language)` → `maxsize=4` (future multilingual, currently unused). `maxsize=4` on country-only loader is a smell. **PR-B1 で残存する global mutable `_cache: ... | None = None` pattern を 3 file(`encounter/protocol.py` / `simulator/helpers.py` / `output/_fhir_diagnostic_report.py`)で撤廃し全て `@lru_cache(maxsize=1)` 統一済**。新規 module で hand-rolled cache を書かないこと(同 pattern は test `cache_clear()` pattern も阻害する)。同 PR で `simulator/helpers.py:_load_all_disease_protocols` の `try/except pass` silent skip も削除済。
