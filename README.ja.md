...
clinosim は **集団からの forward シミュレーション** により合成 EHR データを生成します。各患者は隠れた **14 変数の生理学的状態** (`clinosim/types/clinical.py::PhysiologicalState`) を持ち、全ての観察 (検査、バイタル、投薬、診断) はその状態から導出されます — したがってデータは **構造的に臨床整合** しています。
