...
30 モジュール全体の俯瞰 (依存関係 + 典型的なコールチェーン + 5-step 新モジュール quick-start) は **[`MODULES.md`](MODULES.md)** を参照。
...
最新レビュー:
- [`docs/reviews/2026-07-01-tier1-3-document-density-alpha-min-1-dqr.md`](docs/reviews/2026-07-01-tier1-3-document-density-alpha-min-1-dqr.md) — Tier 1 #3 文書密度 α-min-1: **1/4 軸 PASS** (silent_no_op 17/17 PASS)。DocumentReference 0 → 23,760 (US) / 3,909 (JP); Composition 0 → 9,275 / 474; ClinicalImpression 0 → 23,760 / 3,909。AllergyIntolerance SNOMED 8-field スキーマ upgrade。[AD-63]
- [`docs/reviews/2026-07-01-tier1-3-document-density-alpha-min-2-dqr.md`](docs/reviews/2026-07-01-tier1-3-document-density-alpha-min-2-dqr.md) — Tier 1 #3 文書密度 α-min-2: **2/4 軸 PASS** (clinical PASS + silent_no_op 25/25 PASS)。CareTeam 0 → 158,811 (US) / 16,046 (JP) ★ GAP CLOSED。DocumentReference +22,798(看護シフト記録)。Composition +8,671(看護 2 種)。3 新規 always-on POST_ENCOUNTER Module。[AD-64] — [マスタープラン](docs/design-notes/2026-06-30-tier1-document-and-event-density-master-plan.md) 参照
- [`docs/reviews/2026-06-26-phase-3b-2-hai-susceptibility-data-quality-review.md`](docs/reviews/2026-06-26-phase-3b-2-hai-susceptibility-data-quality-review.md) — Phase 3b-2 HAI culture S/I/R susceptibility chain (**全 3 軸 PASS** + antibiogram firing proof + byte-diff NDJSON IDENTICAL)
...
- **JP language**: Condition / DR / Med / Immunization / care_level / smoking / alcohol で 100% 日本語表示、JLAC10 コードは JCCLS-JSLM 公式日本語名、US locale で日本語混入 0
...
- **FHIR `ServiceRequest` 検査オーダーライフサイクル** (PR1, 2026-06-29) — パネル集約 (CBC/BMP/LFT/ABG/Lipid/Coag/UA): パネル 1 件あたり 1 SR、単独オーダーは 1 SR。JP Core PLAC identifier (HL7 v2-0203)、dual category コーディング (SNOMED 108252007 + v2-0074 LAB)。US p=10k: 362k SR、JP p=5k: 42k SR、パネル SR 5.3%。[AD-61]
- **画像診断メタデータチェーン** (Tier 1 #2, 2026-06-30) — ImagingStudy (DCM モダリティ、マルチシリーズ、urn:dicom:uid 識別子)、Endpoint (WADO-RS URL プレースホルダー、将来の PACS / 画像生成 AI 統合ポイント)、放射線科 DiagnosticReport (所見と印象を `text.div` + `conclusion` に記載)、および画像カテゴリの ServiceRequest (SNOMED 363679005 + v2-0074 RAD)。PR1 スコープ: CR (X 線) + CT モダリティ、胸部 + 頭部部位、細菌性 / 誤嚥性肺炎 + 出血性脳卒中。[AD-62]
- **文書密度チェーン α-min-1** (Tier 1 #3, 2026-07-01) — Stage 1 デフォルトのテンプレート起動による臨床文書 emit: DocumentReference (入院 H&P + 経過記録 + 退院サマリ、LOINC-coded、base64 encoded)、Composition (7 セクション構造の退院サマリ)、ClinicalImpression (日次臨床印象) を全 inpatient/ICU/rehab encounter に出力。AllergyIntolerance は 8-field SNOMED-coded スキーマに upgrade (allergen SNOMED + 反応 + カテゴリ + 重篤度 + 臨床/検証ステータス)。US p=10k: 23,760 DR + 9,275 Composition + 23,760 CI; JP p=5k: 3,909 DR + 474 Composition + 3,909 CI。[AD-63] — [マスタープラン](docs/design-notes/2026-06-30-tier1-document-and-event-density-master-plan.md)
- **文書密度チェーン α-min-2** (Tier 1 #3, 2026-07-01) — CareTeam ★ GAP CLOSED (0 → 158,811 US / 16,046 JP)。担当医 + 看護師 1 名の Participant を持つ encounter-scoped `CareTeam` を POST_ENCOUNTER 3 新規 always-on Module (triage, nursing_assignment, careteam_builder) が生成。看護シフト記録 (LOINC 34746-8 + Composition) + 看護入院アセスメント Composition + 退院看護サマリ Composition (AD-32 対応) を追加。DocumentReference +22,798 / Composition +8,671 (US)。[AD-64]
...
- **Coag パネル起動** (LOINC 24373-3 = aPTT and PT/INR パネル): `APTT`、`PT` (秒、ISI=1.0 で PT = 12 × PT_INR 一貫不変条件)、`Fibrinogen` をすべて既存軸 `coagulation_status` + `inflammation_level` から導出 — APTT は DIC 重症度に比例、PT は既存 PT_INR と数学的に連動、Fibrinogen は **biphasic** (炎症で acute-phase reactant ↑、DIC で消費 ↓)。同 PR で `lab_panels.yaml` に Coag / LFT / Lipid / UA を追加し、出力側 `lab_panel_groups.yaml` との対称性を回復。AD-57 BNP-pattern surgical (公式のみ、新 state 追加なし) + AD-59 per-order RNG 分離により、master との byte-diff (p=2000 seed=42) で Patient/Encounter/Condition/Med/Procedure/Imaging/Immunization/FamilyHistory NDJSON は不変
- **VTE スペクトラム D-dimer** (LOINC 48065-7 / JLAC10 2B140): `D_dimer` を `coagulation_status` + `inflammation_level` + 年齢 + 新 `causes_vte` シナリオフラグ (PE / DVT / 塞栓性脳梗塞に設定) から導出。PE / DVT / 脳梗塞の入院時 D-dimer p50 ≥ 4 ug/mL FEU (臨床的陽性)、敗血症 VTE なしは非特異な p50 < 2 に留まる。出血性脳卒中は機序が異なる (頭蓋内 fibrinolysis = `coagulation_status` で表現) ためフラグ対象外。シナリオフラグ配線を `scenario_flags_from_protocol(protocol)` ヘルパに集約 — `derive_lab_values` への将来の新フラグ追加が全コールサイト (inpatient / ED / outpatient) に自動到達する。本 PR は同時に潜在 defect も解消: `emergency.py` が `causes_myocardial_injury` を渡していなかったため ED 経由 MI 患者がトロポニン上昇しない問題を、上記ヘルパで全配線統一
- **ワルファリン治療域 PT-INR カップリング** (Phase 2b): `PT_INR` の導出に `medication_flags_from_context(patient, medication_orders, admission_date, current_day)` シブリングヘルパを追加。検出は (1) `patient.current_medications` に warfarin / ワルファリン / coumadin が含まれる場合 (AF I48 + 退院後 I26 / I82 / I63 の `chronic_medications.yaml` 経由)、または (2) 院内 warfarin 医薬品オーダーが 3 日以上前 (ローディング 3 日ルール、`all_orders` ピーク経由)。`on_warfarin=True` 時 PT_INR を 2.5 + 半減利得の併存症摂動に上書き — ワルファリン単独患者は治療域 2.0-3.0、肝硬変 (hepatic ↓) や DIC (coag ↑) との併用で 3.0-3.5 (出血リスク可視化)、DOAC (アピキサバン / リバーロキサバン / エドキサバン / ダビガトラン) 患者は意図的に検出しない (DOAC では INR を臨床的にモニターしない実態に忠実)。US p=10000 監査: warfarin p50 INR 2.70 (治療域)、DOAC p50 1.80 ≈ no-AC p50 1.70 (DOAC 正しく未シフト)。`**flags` マージは scenario flags と同型 — 将来の医薬品-検査値カップリング (ステロイド → グルコース等) はヘルパ一箇所の編集で全コールサイトに到達
...
├── ServiceRequest.ndjson           # 検査オーダー (パネル集約: CBC/BMP/LFT 等 1 SR; 単独オーダー 1 SR) [AD-61]
│                                   # + 画像オーダー (オーダー 1 件あたり 1 SR、SNOMED 363679005 + v2-0074 RAD) [AD-62]
├── ImagingStudy.ndjson             # 放射線科検査 (urn:dicom:uid、DCM モダリティ、マルチシリーズ) [AD-62]
├── Endpoint.ndjson                 # ImagingStudy 単位の WADO-RS URL プレースホルダー (将来の PACS / 画像生成 AI 統合) [AD-62]
├── DiagnosticReport.ndjson         # 検査パネルレポート (CBC/BMP/LFT/Lipid/Coag/UA/ABG, LOINC) + 微生物培養レポート (+ Specimen)
│                                   # + 放射線科読影レポート (所見 + 印象を text.div + conclusion に記載) [AD-62]
