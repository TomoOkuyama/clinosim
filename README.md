...
- **CBC / BMP panel orders emit canonical children** with **per-specimen RNG isolation**: a `{test:"CBC"}` admission order produces WBC + Hb + Hct + Plt as four child Observations from one specimen (and `{test:"BMP"}` produces six emit-able components from one specimen), with the panel's specimen-rejection / hemolysis draws sourced from a per-parent sub-RNG (`panel_specimen_seed(parent_order_id)`) so a panel registry edit cannot cascade into unrelated patients' cohorts (AD-16). Per-panel `min_components` thresholds follow the canonical-N − 1 rule (CBC = 3, BMP = 5) — validated by audit, suppressing 81 % of accidental CBC and 48 % of accidental BMP DRs that the prior thresholds let through
