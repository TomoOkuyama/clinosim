...
- `clinosim/modules/observation/microbiology.py` — `antibiotic_loinc_lookup()`(`ANTIBIOTIC_LOINC_LOOKUP` の single source of truth)
...
1. **NHSN R-rate** per (hai_type, **organism**, antibiotic) cohort — `_NHSN_RESISTANCE_BANDS` 配線、MRSA / ESBL+ / etc. の population-level R-rate を NHSN AR 2018-2020 band で gate。**PR3b-5 D1 完成 (2026-06-29)** で specimen-based join + HAI-only filter に refinement:`_organism_per_specimen` が `Observation.specimen.reference` を joining key にし、`_hai_specimens` が `HAI_EVENT_ID_SYSTEM` identifier で HAI-derived specimens を filter(community culture exclude、C1 multi-organism encounter double-count + C2 community/HAI culture co-occurrence の両 attribution defect を mechanically 解消)
2. **empty susceptibility rate** per **panel-eligible** HAI cohort — `HAI_EMPTY_SUSCEPTIBILITIES_MAX_RATE` 配線、`_panel_eligible_organisms`(`load_hai_antibiogram()` keys から derive)で denominator を panel-eligible encounter に絞り、empty rate ≤ 5%。PR3b-3 D2 完成 (2026-06-29):E.faecalis (78065002) / C.albicans (53326005) が antibiogram 不在で auto-exclude(hard-coded list 不要)、NHSN denominator definition 復元
3. **narrow rate** per hai_type aggregate — `_NARROW_RATE_BANDS`、narrow が発火した event 比率を band で gate(CLABSI [0.60-1.00] / CAUTI [0.50-1.00] / VAP [0.60-1.00] = aggregate per hai_type、vancomycin always-S ELIMINATION + ladder SWITCH の合算)。adversarial-1 C-1 fix で per-(hai_type, organism) → per-hai_type に realign
...
- `_NHSN_RESISTANCE_BANDS: list[dict]` ✓ **active enforcement complete (D1, 2026-06-29)** — CDC NHSN AR 2018-2020 推奨バンド (CLABSI MRSA 40-55%、CAUTI ESBL 12-22%、VAP MRSA 30-45%)。clinical axis の R-rate gate で per-(hai_type, **organism**, antibiotic) cohort 単位に enforcement。`_organism_per_encounter` で per-organism filter 実装済、TODO marker 削除。
- `HAI_EMPTY_SUSCEPTIBILITIES_MAX_RATE: float = 0.05` ✓ **active enforcement complete (D2, 2026-06-29)** — susceptibilities が空の MicrobiologyResult の比率上限。clinical axis の empty-rate gate で **panel-eligible** HAI cohort 単位に enforcement。`_panel_eligible_organisms` で E.faecalis / C.albicans を antibiogram 不在で auto-exclude、TODO marker 削除。
