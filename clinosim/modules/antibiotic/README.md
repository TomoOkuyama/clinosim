...
1. **NHSN R-rate** per (hai_type, **organism**, antibiotic) cohort — `_NHSN_RESISTANCE_BANDS` 配線、MRSA / ESBL+ / etc. の population-level R-rate を NHSN AR 2018-2020 band で gate。PR3b-3 D1 完成 (2026-06-29) で per-organism filter 実装:`_organism_per_encounter` が `Observation.ndjson` mb-org-* を 1 pass 走査して `{enc_id: {organism_snomed, ...}}` を build、cohort encounter を per-organism で絞る(`clabsi/3092008/cefazolin` = S.aureus only、mixed S.aureus + S.epidermidis + E.coli を排除)
