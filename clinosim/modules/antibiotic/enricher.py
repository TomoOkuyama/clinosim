        ...
        for ev in hai_events:
            ...
            for regimen in build_regimens(ev, start_datetime=start_dt):
                ...
                order = Order(
                    ...
                    display_name=ANTIBIOTIC_DRUGS.get(regimen.drug_key, {}).get("name", regimen.drug_key),
...
def _truncate_mar(record, regimen: AntibioticRegimen) -> None:
    ...
    kept = [m for m in mars if not (m.order_id == order_id and m.scheduled_datetime > regimen.discontinuation_datetime)]
