...
├── __init__.py            # Public API export (__all__、N-3 で整備)
├── README.md              # 本ドキュメント
├── SPEC.md
├── engine.py              # LLMService (generate / complete_prompt, template fallbacks)
...
└── prompts/               # Per-language prompt YAML templates (AD-40)
    ├── en/                # English (6 files; narrative_seed.yaml は N-3 追加)
    └── ja/                # Japanese (6 files, AD-43)
```
...
class LLMService:
    ...
    def complete_prompt(
        ...
        language: str, task_type: LLMTaskType,
...
- `generate()` — 構造化イベント入力(`ClinicalEventData`)。task type に応じて
  JUDGMENT/NARRATIVE のカテゴリが決まり、適切な provider と言語が選択される。
  失敗時は template fallback(`fallback_reason` 付き)。
- `complete_prompt()`(N-2, N-chain)— 事前構築 (system, user) prompt の raw 経路。
  retry + PromptCache + token 集計は同じだが **template fallback なし** —
  provider 未構成 / retry 枯渇時は `LLMCompletionError` を raise し、fallback は
  呼び手(例: `LLMNarrativeGenerator`)の責務。narrative section 置換
  (`apply_replacement_strategy` → `prompts/{en,ja}/narrative_seed.yaml`)が使用。
...
| 11506-3 | `PROGRESS_NOTE` | 経過記録 | 入院 LOS 日ごと |
...
| 69730-0 | `DEATH_SUMMARY` | 死亡診断書 | 死亡退院 |
| 78390-2 | `ADMISSION_NURSING_ASSESSMENT` | 入院時看護アセスメント | 全入院(α-min-2) |
| 34746-8 | `NURSING_SHIFT_NOTE` | 看護記録(3 交代) | LOS 日 × 3(α-min-3) |
| 34745-0 | `NURSING_DISCHARGE_SUMMARY` | 看護退院サマリー | 全退院(α-min-2) |
| 34131-3 | `OUTPATIENT_SOAP` | 外来 SOAP | 外来受診(α-min-2) |
| 34878-9 | `ED_NOTE` | 救急外来記録 | ED 受診(α-min-2) |
| 54094-8 | `ED_TRIAGE_NOTE` | トリアージ記録 | ED 受診(α-min-2) |

**enum sync invariant(N-3)**: `clinosim.types.document.DocumentType` の全値は
NARRATIVE category の `LLMTaskType` 値として存在しなければならない
(`engine._validate_document_task_sync` が import 時に検証、破ると `ImportError`)。
逆方向(`death_summary` 等 LLMTaskType-only)は将来 phase の予約として許容。
`DOCUMENT_LOINC` ↔ `document_type_specs.yaml` の LOINC 一致は
`tests/unit/test_llm_task_enum_sync.py` が pin。coarse `NURSING_NOTE` は N-3 で削除済。
