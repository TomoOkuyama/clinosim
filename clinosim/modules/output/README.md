...
├── narrative_generator.py   # Stage 2: 構造 CIF → narrative テキスト
...
### `generate_narratives(cif_dir, llm_service, version_id=None, language="ja") -> str`

構造 CIF を読み、 各患者について自由記述ノートを生成する (Stage 2)。 LLM は `LLMService` 経由のため、 mode を `"none"` / `"template"` / `"llm"` で切り替えられる (AD-11)。

**生成されるノート種別**:

- `admission_hp` — 入院時 H&P (1 通)
- `progress_note` — Day 1, 3, 7, 退院前日の経過記録 (SOAP)
- `discharge_summary` — 退院時サマリー (1 通)

**バージョン管理**: `narratives/<version_id>/patients/` 配下に 1 患者 1 ファイルで保存し、 同 dir に `manifest.json` (LLM mode, モデル, コスト) と、 親に `current_version.txt` を書き出す。 同じ構造 CIF に対して異なる LLM で複数バージョンを生成・保持できる。
...
version_id = generate_narratives("./output/run_001", llm, language="ja")
# → "narrative_20260406_103000"
```
...
- v0.1-alpha では Bundle 形式 (`Bundle.entry[]` ラップ) は出力しない。 NDJSON のみ
- `MedicationStatement` (在宅薬), `CarePlan`, `Goal` は未実装
- `DocumentReference` による narrative ノートの埋め込みは未対応 (narratives JSON は別ファイルで保持)
