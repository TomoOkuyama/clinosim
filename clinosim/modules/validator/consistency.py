...
def _check_discharge_hgb(record: CIFPatientRecord, pid: str, report: ConsistencyReport) -> None:
    ...
    if record.deceased and enc.discharge_disposition != "exp":
        report.add(
            ConsistencyIssue(
                pid,
                "error",
                "deceased_disposition",
                f"Deceased but disposition={enc.discharge_disposition} (expected 'exp')",
            )
        )
    if not record.deceased and enc.discharge_disposition == "exp":
        report.add(
            ConsistencyIssue(
                pid,
                "error",
                "alive_but_expired",
                "Not deceased but disposition='exp'",
            )
        )


def _check_lab_ranges(record: CIFPatientRecord, pid: str, report: ConsistencyReport) -> None:
...
def _check_medication_holds(record: CIFPatientRecord, pid: str, report: ConsistencyReport) -> None:
    ...
    if all_conditions & metformin_hold_diseases:
        ...
        for order in record.orders:
            ...
            if hasattr(order, "status") and order.status.value in ("cancelled", "discontinued", "stopped"):
                continue
...
def _check_procedure_fields(record: CIFPatientRecord, pid: str, report: ConsistencyReport) -> None:
...
def _check_los_consistency(record: CIFPatientRecord, pid: str, report: ConsistencyReport) -> None:
...
def _check_vital_ranges(record: CIFPatientRecord, pid: str, report: ConsistencyReport) -> None:
...
def _check_sex_specific_conditions(record: CIFPatientRecord, pid: str, report: ConsistencyReport) -> None:
