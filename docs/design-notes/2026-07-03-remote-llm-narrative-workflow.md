...
pip install -e ".[dev,llm]"    # dev = pytest 等 / llm = httpx (ollama provider)
...
**モデル名は設定 YAML と一致させること。** デフォルト設定
`clinosim/config/llm_service.yaml` のモデルは **`llama3.1:8b`**
(`narrative.local.model`)。名前が一致しないと全 call が 404 →
**全 doc が silent に template fallback** になる(症状は §8 参照)。
選択肢は 3 つ:

```bash
# (a) デフォルト設定のままモデルを合わせる
ollama pull llama3.1:8b
```

```bash
# (b) 別モデル(例: session 32 smoke で実績のある gpt-oss:20b)を pull して
#     llm_service.yaml の narrative.local.model と model_map を編集する
ollama pull gpt-oss:20b
```

```yaml
# (c) 別ファイルを書いて narrate に --llm-config my_llm.yaml で渡す
narrative:
  mode: "llm"
  provider: "local"
  local:
    endpoint: "http://localhost:11434"
    model: "<pull したモデル名>"   # 例: gpt-oss:20b
```
...
```bash
# 初回(fresh version dir へは flag 不要)
...
# 2 回目以降(既存 version dir への filtered 書込みは明示 opt-in が必要)
clinosim narrate --cif-dir /tmp/jp100/cif --provider ollama --country JP \
    --version-id trial-7 --patient-filter "ENC-POP-000002" --merge-into-version
```

- filter は patient JSON の **filename stem と patient_id の両方**に対する
  regex(`ENC-A|POP-000777` のような alternation 可)。
- 部分実行 version は manifest に `patient_filter` + `partial: true` が
  記録され自己記述的。
- **filter 付き実行は provider に関係なく set-current しない**(adv-1 I-1
  default)— 部分 version が silent に "current" になることはない。明示
  `--set-current` は通るが stderr WARNING が出る。
- **既存 documents を持つ version dir への filtered 書込みは
  `--merge-into-version` なしでは exit 2 で拒否**(前回生成の stale file が
  混在する mixed version 防止)。opt-in 時は前回 run の file が残り、
  manifest は最後の run のみを反映する(stderr NOTICE)。
...
| 症状 | 見る場所 | 対処 |
|---|---|---|
| 全 doc が template のまま | stderr WARNING + manifest `generator_fallback_docs` | provider 起動を確認 / model 名一致を確認(`ollama list` の名前と `narrative.local.model` — §1 参照) |
