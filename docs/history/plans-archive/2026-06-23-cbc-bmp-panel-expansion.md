...
> **⚠ REVISED MID-EXECUTION 2026-06-23.** Tasks 1–3 below landed unchanged. After Task 3 the byte-diff verification revealed a pre-existing AD-16 violation in `_run_daily_loop` (panel children draw specimen rejection from the patient-scoped master RNG, so adding a registry entry cascades into unrelated patients). The fix in `inpatient.py` requires a Python source change and shifts ABG Observation values — neither of which fits the original "YAML-only, additions-only" frame. The actual implemented scope is recorded in the spec; this plan retains Tasks 1–3 as faithfully executed and replaces Tasks 4–6 with a single Task 4-R note pointing back to the spec for the final structure. See the spec §1.2 / §3 / §4 for what shipped.

**Goal (original):** Register CBC and BMP in the observation panel-expansion registry so 9 currently-silently-dropped `{test: "CBC"}` / `{test: "BMP"}` orders in 4 disease protocols (cerebral_infarction, deep_vein_thrombosis, hemorrhagic_stroke, diabetic_ketoacidosis) emit canonical child component Observations and FHIR DiagnosticReports, while keeping every other NDJSON byte-identical.

**Goal (revised):** Same as above for the data-quality outcome **plus** make the lab-resulting loop's specimen-rejection / hemolysis / staff-assignment draws panel-children-isolated so a panel-registry edit does not cascade into the master patient-scoped RNG stream (AD-16) and so specimen rejection becomes per-specimen rather than per-analyte (clinically correct).

**Architecture (original — kept for Tasks 1–3):** Two YAML edits (one registry add, one comment correction), four pytest cases (unit ×2 in the panel registry, integration ×2 in the simulator pipeline), one byte-diff verification script in `scratchpad/`, and one audit-trail markdown. No Python source changes. No physiology, RNG, CIF, or order-engine changes. The fix relies on the existing ABG panel-expansion code path at `clinosim/simulator/inpatient.py:572-585`.

**Architecture (revised — what shipped):** Tasks 1–3 above plus a `panel_specimen_seed` helper in `clinosim/simulator/seeding.py`, a two-pass split of the lab-resulting loop in `clinosim/simulator/inpatient.py` (Pass 1 = master RNG, non-panel-child orders; Pass 2 = panel children, per-parent isolated sub-RNG), extended integration tests covering per-specimen rejection and Cl/Ca silent drop, a relaxed byte-diff invariant (cohort drift ≤ 1% is acceptable; strict-grow on the lab-touching files), and a data-quality audit doc replacing the original "additions-only" assertion.
