...
def run_simulator(cwd: Path, out_dir: Path, country: str, n: int, seed: int) -> None:
    ...
    cmd = [
        ...
        "--format",
        "fhir",
        "csv",  # nargs="+" — multiple format tokens
    ]
...
def compare(branch_dir: Path, master_dir: Path, label: str) -> int:
    ...
    for fname in EXPECT_IDENTICAL:
        ...
        print(f"  [{status}] IDENTICAL  {fname}: master={m_lines} / branch={b_lines}")
    ...
    for fname in EXPECT_DIFF:
        ...
        if b is None or m is None:
            failures += 1
            print(f"  [FAIL] DIFF-OK    {fname}: missing on one side (master={m is not None}, branch={b is not None})")
            continue
        ...
        if status == "FAIL":
            failures += 1
        print(f"  [{status}] DIFF-OK    {fname}: master={m[1]} → branch={b[1]} ({b[1] - m[1]:+d} lines)")

    # Any branch file not categorized → unexpected diff.
    extra = (set(branch_hashes) | set(master_hashes)) - set(EXPECT_IDENTICAL) - set(EXPECT_DIFF)
    for fname in sorted(extra):
        ...
        m_lines = m[1] if m else 0
        b_lines = b[1] if b else 0
        print(f"  [FAIL] UNEXPECTED  {fname} differs (not in either bucket): master={m_lines} branch={b_lines}")
...
def main() -> None:
    ...
    subprocess.run(["git", "worktree", "add", str(master_wt), "master"], check=True, cwd=REPO)
    try:
        print("\n== master (worktree) ==")
        run_simulator(master_wt, master_us, "US", 2000, 42)
        run_simulator(master_wt, master_jp, "JP", 1000, 42)
    finally:
        subprocess.run(["git", "worktree", "remove", "--force", str(master_wt)], check=True, cwd=REPO)
    ...
    for label, b, m in [
        ...
        if b.exists() and m.exists():
            ...
            print(f"\nSKIP {label}: directory missing (branch={b.exists()}, master={m.exists()})")
