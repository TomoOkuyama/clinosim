...
### 6.2 Encounter(3 ファイル)

| ファイル | 設定値 | 根拠 |
|----------|--------|------|
| `viral_gastroenteritis.yaml` | -0.5 | 下痢主体 = HCO3 stool loss = non-AG hyperchloremic acidosis(教科書典型) |
| `food_poisoning.yaml` | -0.4 | 同上(嘔吐優位で alkalosis 寄りの場合あり、平均化) |
| `chemical_exposure.yaml` | 0.2 | 中毒物質次第(平均化)。methanol/ethylene glycol 等は AG ↑(該当しない場合は default で OK) |
...
| リスク | 緩和 |
|--------|------|
| disease YAML 編集が他テストを壊す | byte-diff invariant gate(physiology state 変数の cascade 検知) |
...
| AG 軸が `apply_coupling_rules` で副作用 | rule に AG → other state の path を一切書かない(本 spec で禁止)+ test_anion_gap_status_does_not_mutate_state で守る |
