...
- 新 `modules/antibiotic/` Module (AD-55 Module、opt-in、`SimulatorConfig.modules` で有効化)
...
| Opt-in OFF default で既存 golden 不変保証 | (1) `register_builtin_enrichers` で `if module_enabled("antibiotic"):` ガード、(2) byte-diff scope に opt-in OFF + ON 両方を含める |
