...
5. **Backward compatibility** — 128 caller sites must keep working. `_fhir_common.py` shim (from #545) stays. `fhir_r4_adapter.py` becomes a thin re-export shim so its 100+ callers keep the same import path.
...
├── _fhir_common.py                 — deprecated shim from #545 (retargeted in PR1 to fhir_r4/lib/common)
...
└── fhir_r4/                        — new: FHIR R4 output subsystem
    ...
    ├── lib/                        — shared FHIR helpers (6 files, PR1)
    │   ├── __init__.py
    ...
    ├── demographics/               — patient, practitioner, family_history, smoking_alcohol (4 files, PR2)
    ├── encounters/                 — encounter, care_team, care_level, facility, endpoint (5 files, PR2)
    ├── medications/                — medications (1 file, 60k, PR2)
    ├── labs/                       — observations, diagnostic_report, service_request, microbiology, imaging_study, coding_package, coding_strategy (7 files, PR2)
    ├── procedures/                 — procedures, immunization, device, nursing (4 files, PR2)
    ├── conditions/                 — conditions, allergy_intolerance, clinical_impression, hai, code_status (5 files, PR2)
    ├── documents/                  — composition, documents, document_reference_checkup (3 files, PR2)
    └── post_process/               — datetime_normalize, jp_ecs, strip, specimen, profile (5 files, PR3 — folds #556)
...
- **`fhir_r4_adapter.py`** becomes a thin shim at `output/` root: 100+ caller sites won't break; the real facade lives inside `fhir_r4/__init__.py`.
- **FHIR support files** move under `fhir_r4/lib/` (PR1) or into their clinical-domain subdir (PR2):
  - `opaque_ids.py` → `fhir_r4/lib/ids.py` (Resource.id is a FHIR construct, used by all builders; belongs in the shared lib)
  - `lab_coding_package.py` and `_lab_coding_strategy.py` → `fhir_r4/labs/coding_package.py` and `fhir_r4/labs/coding_strategy.py` (JP-CLINS lab-specific, only labs/ code uses them; PR2 scope)
...
| `from clinosim.modules.output._fhir_common import <anything>` | Continues to work via existing #545 shim (with `DeprecationWarning`), retargeted to `fhir_r4/lib/common`. |
...
- Update all `clinosim/` and `tests/` callers importing from the 6 moved files (~83 files, ~148 import statements).
...
**Expected diff**: ~6 file mv + 1 facade move + shims + ~83 caller updates. Medium.
...
- Audit `_fhir_medications.py:296` (`Path(__file__).resolve().parents[2]`) and `lab_coding_package.py:545` (`parents[3]`) — both must have their parent-depth adjusted for the new location (this is the class of bug that broke the closed PR #604).
...
**Verification**: full protocol above. Special attention to `git mv` preserving history AND `Path(__file__).parents[N]` fixes.
...
### Layout A: role-based (Issue #555's original proposal — attempted by closed PR #604)

`fhir_r4/builders/*.py` (25+ files) + `fhir_r4/post_process/` + `fhir_r4/*.py` shared library.

Rejected because: `builders/` becomes another flat directory of 25 files. Clinical domain grouping is stronger on all 5 stated axes. PR #604 also failed to fix `Path(__file__).parents[N]` regressions and mislabeled its own test-failure delta as "pre-existing" (see the [PR #604 close comment](https://github.com/TomoOkuyama/clinosim/pull/604#issuecomment-5224346643) for the full analysis).
