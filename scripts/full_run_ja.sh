#!/usr/bin/env bash
# Full JP run: CIF generation + narrative generation (Bedrock)
# Run on EC2 with Bedrock access.
#
# Usage (session-safe):
#   nohup ./scripts/full_run_ja.sh > /dev/null 2>&1 &
#   tail -f test_data/bedrock_ja_full_results.txt
set -euo pipefail

OUTPUT_DIR="output/jp_full"
VERSION_ID="bedrock_ja_full"
LLM_CONFIG="clinosim/config/llm_service.bedrock.yaml"
RESULT_FILE="test_data/bedrock_ja_full_results.txt"

{
echo "============================================================"
echo "  Full JP Run (CIF + Narrative)"
echo "  Date: $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
echo "  PID: $$"
echo "============================================================"
echo ""

# Step 1: Generate CIF (JP, 5K population, 50-bed hospital, seed=42)
...
echo ""
echo ">>> Step 2: Narrative generation (Bedrock)"
