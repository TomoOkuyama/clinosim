...
def mock_cif_dir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    gen = _run_cli(
        "test-disease",
        "--patient-profile",
        str(FIXTURE_DIR / f"{PROFILE_ID}.yaml"),
        "--format",
        "cif",
        "-o",
        str(out),
...
def test_check_narratives_mock_pipeline_green(mock_cif_dir: Path, tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, f"check-narratives failed on the mock pipeline:\n{result.stdout}\n{result.stderr}"
...
@pytest.mark.integration
def test_check_narratives_broken_expectations_exit_1(mock_cif_dir: Path, tmp_path: Path) -> None:
...
@pytest.mark.integration
def test_check_narratives_invalid_expectations_exit_2(mock_cif_dir: Path, tmp_path: Path) -> None:
