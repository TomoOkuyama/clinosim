...
def jp_export_dir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    out = tmp_path_factory.mktemp("jp_imaging") / "out"
    run_generate("JP", 200, 42, out)
    return out


@pytest.mark.integration
def test_jp_imaging_study_modality_dcm_display_absent(jp_export_dir: Path) -> None:
    ...
    """
    studies = load_ndjson(find_ndjson(jp_export_dir, "ImagingStudy.ndjson"))
    if not studies:
...
@pytest.mark.integration
def test_jp_imaging_study_series_modality_dcm_display_absent(jp_export_dir: Path) -> None:
    ...
    for study in studies:
        for series in study.get("series", []):
            ...
            display = mod.get("display", "")
            assert not _has_jp_chars(display), (
                f"ImagingStudy/{study['id']} series modality display leaked Japanese under DICOM system: {display!r}"
            )


@pytest.mark.integration
def test_jp_radiology_dr_conclusion_in_ja(jp_export_dir: Path) -> None:
    """JP cohort: radiology DiagnosticReport.conclusion must be Japanese."""
    drs = load_ndjson(find_ndjson(jp_export_dir, "DiagnosticReport.ndjson"))
...
@pytest.mark.integration
def test_jp_radiology_dr_text_div_in_ja(jp_export_dir: Path) -> None:
...
@pytest.mark.integration
def test_jp_imaging_sr_category_ja_label_via_text(jp_export_dir: Path) -> None:
    ...
    """
    srs = load_ndjson(find_ndjson(jp_export_dir, "ServiceRequest.ndjson"))
    imaging_srs = [
        s
        for s in srs
