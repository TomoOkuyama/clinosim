    ...
    for category, entries in a.items():
        for e in entries:
            for r in e.get("common_reactions", []):
                ...
                assert "manifestation_display_en" in r, f"{category}: missing manifestation_display_en"
...
def test_prevalence_adult_in_range():
    ...
    for category, entries in a.items():
        for e in entries:
            ...
            assert 0 <= adult <= 1, f"{category}[{e['allergen_display_en']}]: prevalence.adult={adult} out of range"
