...
def test_chronic_primary_resolves_to_chronic_id() -> None:
    ...
    assert primary_condition_ref(_record("I10", ["I10", "E11"]), "PAT", "ENC-1") == "cond-chronic-PAT-00"
    # Chronic at index 1
    assert primary_condition_ref(_record("E11.9", ["I10", "E11"]), "PAT", "ENC-1") == "cond-chronic-PAT-01"


def test_specific_dx_matches_base_chronic() -> None:
    """Encounter dx `I50.9` matches chronic `I50` (base I50) → chronic id."""
    assert primary_condition_ref(_record("I50.9", ["I50"]), "PAT", "ENC-HF") == "cond-chronic-PAT-00"


def test_base_chronic_matches_specific_dx() -> None:
    """Encounter dx `I50` matches chronic `I50.9` (base I50) → chronic id."""
    assert primary_condition_ref(_record("I50", ["I50.9"]), "PAT", "ENC-HF") == "cond-chronic-PAT-00"
...
def test_admission_dx_fallback_when_discharge_missing() -> None:
    ...
    assert primary_condition_ref(record, "PAT", "ENC-1") == "cond-chronic-PAT-00"
...
def test_from_codes_variant_matches_full_record_variant() -> None:
    ...
    assert from_full == from_codes == "cond-chronic-PAT-00"


def test_from_codes_variant_handles_none_and_empty() -> None:
    assert primary_condition_ref_from_codes("Z00.0", None, "PAT", "ENC-1") == "cond-ENC-1-primary"
    assert primary_condition_ref_from_codes("Z00.0", [], "PAT", "ENC-1") == "cond-ENC-1-primary"
    assert primary_condition_ref_from_codes("", ["I10"], "PAT", "ENC-1") == "cond-ENC-1-primary"
