...
def _make_args(**kwargs) -> SimpleNamespace:
    ...
    base = dict(
        disease_id=None, patient_profile=None, count=None, severity=None,
        archetype=None, seed=None, country=None, format=None, output=None,
    )
    base.update(kwargs)
...
def test_cli_seed_42_overrides_profile_seed_end_to_end(tmp_path: Path):
    ...
    profile_path = _write_profile(tmp_path, "seed_override_e2e", {
...
def test_omitted_seed_uses_profile_seed_end_to_end(tmp_path: Path):
    ...
    profile_path = _write_profile(tmp_path, "seed_profile_e2e", {
