...
def test_regenerate_single_profile(tmp_path: Path):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "clinosim.simulator.cli", "regenerate-goldens", "--profile", "single_test"],
        capture_output=True,
        text=True,
        check=False,
...
def test_profile_and_all_mutually_exclusive(tmp_path: Path):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "clinosim.simulator.cli", "regenerate-goldens", "--profile", "x", "--all"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_regenerate_is_idempotent(tmp_path: Path):
    ...
    subprocess.run(
        [sys.executable, "-m", "clinosim.simulator.cli", "regenerate-goldens", "--profile", "idem_test"],
        env=env,
        capture_output=True,
        text=True,
        check=True,
...
def test_regenerate_mock_provider_writes_llm_mock_golden(tmp_path: Path):
    ...
    assert not (fixture_dir / "mock_test.golden.json").is_file(), "--provider mock must NOT touch the template golden"
    golden = json.loads(golden_path.read_text())
...
def test_regenerate_template_rejects_llm_only_flags(tmp_path: Path):
    ...
    for extra in (["--model-tag", "x"], ["--llm-config", "/tmp/nonexistent.yaml"]):
        result = subprocess.run(
            [sys.executable, "-m", "clinosim.simulator.cli", "regenerate-goldens", "--profile", "whatever", *extra],
            capture_output=True,
            text=True,
            check=False,
            env=_env_with(fixture_dir),
