...
def test_no_duplicate_offsets():
    ...
    assert len(set(values)) == len(values), f"duplicate ENRICHER_SEED_OFFSETS: {ENRICHER_SEED_OFFSETS!r}"


@pytest.mark.unit
def test_all_modules_registered():
    expected = {"identity", "microbiology", "immunization", "code_status", "family_history", "care_level", "nursing"}
...
def test_hex_ascii_convention_new_modules():
    ...
    for name, offset in ENRICHER_SEED_OFFSETS.items():
        if name in grandfathered:
            continue
        assert offset < 0x10000, f"{name} offset {offset:#x} exceeds 16-bit hex ASCII range"
...
def test_hex_ascii_values_match_module_names():
    ...
    for name, expected in expected_ascii.items():
        assert ENRICHER_SEED_OFFSETS[name] == expected, f"{name}: {ENRICHER_SEED_OFFSETS[name]:#x} != {expected:#x}"
