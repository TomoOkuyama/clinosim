...
def _stage(code, stage_text):
    record = {
        "patient": {
            "chronic_conditions": [
                ChronicCondition(code=code, onset_date="2020-01-01", severity="moderate", stage=stage_text),
            ],
        },
...
def test_stage_summary_coding_uses_jp_display():
    ...
    c = next(x for x in _build_conditions(record, "P1", "JP") if x["code"]["coding"][0]["code"].startswith("N18"))
...
def test_every_generated_stage_is_mapped():
    ...
    from clinosim.modules.output.fhir_r4.conditions.conditions import _STAGE_SUMMARY_SNOMED
