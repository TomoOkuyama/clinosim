...
def test_dict_protocol_reads_both_flags():
    flags = scenario_flags_from_protocol({"causes_myocardial_injury": True, "causes_vte": True})
...
def test_keys_match_derive_lab_values_parameter_names():
    ...
    for key in flags:
        assert key in sig.parameters, (
            f"scenario_flags_from_protocol returned key '{key}' that is not a derive_lab_values parameter"
        )
